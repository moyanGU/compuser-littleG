...
async function main(): Promise<void> {
  ...
  try {
    ...
    let editorWindowTitle = selectEditorWindowTitle(snapshotData)
    let chatWindowTitle = selectChatWindowTitle(snapshotData)

    if (!editorWindowTitle || !chatWindowTitle || liveWindowTargets.length < 2) {
      await ensureLiveSmokeWindowTargets(runtime, context)
      snapshotData = await refreshLiveSmokeSnapshot(runtime, context)
      liveWindowTargets = selectDistinctLiveSmokeWindows(snapshotData)
      editorWindowTitle = selectEditorWindowTitle(snapshotData)
      chatWindowTitle = selectChatWindowTitle(snapshotData)
    }
    ...
    const browserTemplateResult = await runtime.execute(
      {
        ...
        input: {
          ...
          chatAppName: 'Codex',
          chatTargetWindowTitle: chatWindowTitle ?? 'Codex',
        },
      },
      context,
    )

    reportOptionalBrowserEditorChatTemplateScenario(browserTemplateResult)

    const liveFixturePath = resolve(
      context.cwd,
      'tmp',
      'phase3-live-file-browser-desktop.txt',
    )
    await writeFile(
      liveFixturePath,
      ['phase3 live smoke', 'browser-routed desktop template'].join('\n'),
      'utf8',
    )
    ...
    if (liveWindowTargets.length < 2) {
      console.log(
        '[skip] phase3-live-smoke file-browser-desktop-template environment_unready requires at least two distinct confirmable windows',
      )
      console.log(
        '[skip] phase3-live-smoke multi-window-observe-route-deliver-template environment_unready requires at least two distinct confirmable windows',
      )
      return
    }
    ...
    const multiWindowTemplateResult = await runtime.execute(
      {
        ...
        input: {
          ...
          actionText: 'phase3 live smoke',
        },
      },
      context,
    )

    reportOptionalMultiWindowTemplateScenario(
    ...
    if (!browserRouteTargets) {
      console.log(
        '[skip] phase3-live-smoke file-browser-desktop-template environment_unready requires one text-capable target window plus one distinct comparison window',
      )
      return
    }
    ...
    reportOptionalFileBrowserDesktopTemplateScenario(
...
function assertPermissionMode(permissionMode: SmokeOptions['permissionMode']): void {
  if (permissionMode === 'read-only') {
    throw new Error(
      'permission_blocked permission mode read-only does not allow phase3 template smoke actions',
    )
  }
}
...
function reportOptionalBrowserEditorChatTemplateScenario(result: {
  ok: boolean
  data?: unknown
  summary: string
}): void {
  if (result.ok) {
    ...
    assertOperationPresent(
      ...
      'verification_failed browser editor chat template base chain missing',
    )
    ...
    assertEvidencePresent(
      result.data,
      'verified:',
      'verification_failed browser editor chat template verified delivery evidence missing',
    )
    console.log('[pass] phase3-live-smoke browser-editor-chat-template')
    return
  }

  if (isOptionalBrowserEnvironmentUnready(result)) {
    console.log(
      `[skip] phase3-live-smoke browser-editor-chat-template environment_unready ${result.summary}`,
    )
    return
  }

  if (isOptionalBrowserEditorTargetEnvironmentUnready(result)) {
    console.log(
      `[skip] phase3-live-smoke browser-editor-chat-template environment_unready ${result.summary}`,
    )
    return
  ...
  throw new Error('verification_failed browser editor chat template chain failed')
}

function reportOptionalFileBrowserDesktopTemplateScenario(
  result: {
    ok: boolean
    data?: unknown
    summary: string
  ...
  if (result.ok) {
    ...
    assertSelectedTarget(
      result.data,
      expectedTarget,
      'verification_failed file browser desktop template selected unexpected target',
    )
    console.log(
      `[pass] phase3-live-smoke file-browser-desktop-template routeQuery=${routeQuery} -> ${expectedTarget}`,
    )
    return
  }
  ...
  if (isOptionalBrowserRouteEnvironmentUnready(result)) {
    console.log(
      `[skip] phase3-live-smoke file-browser-desktop-template environment_unready ${result.summary}`,
    )
    return
  }

  throw new Error('verification_failed file browser desktop template chain failed')
}

function reportOptionalMultiWindowTemplateScenario(result: {
  ok: boolean
  data?: unknown
  summary: string
}, expectedTarget: string, routeQuery: string): void {
  if (result.ok) {
    assertVerificationPassed(
      result.data,
    ...
    assertSelectedTarget(
      result.data,
      expectedTarget,
      'verification_failed multi-window route deliver template selected unexpected target',
    )
    console.log(
      `[pass] phase3-live-smoke multi-window-observe-route-deliver-template routeQuery=${routeQuery} -> ${expectedTarget}`,
    )
    return
  }
  ...
  if (isOptionalMultiWindowEnvironmentUnready(result)) {
    console.log(
      `[skip] phase3-live-smoke multi-window-observe-route-deliver-template environment_unready ${result.summary}`,
    )
    return
  }

  throw new Error('verification_failed multi-window route deliver template chain failed')
}
...
async function refreshLiveSmokeSnapshot(
  ...
  assert(snapshot.ok, 'verification_failed phase3 snapshot failed')
...
async function prepareBrowserSmokeFixture(
  ...
  const fixturePath = resolve(
    context.cwd,
    'tmp',
    'phase3-live-browser-fixture.html',
  )
...
function selectDistinctLiveSmokeWindows(data: unknown): string[] {
  const values = readLiveSmokeWindowTitles(data)
  const result: string[] = []
  const seen = new Set<string>()
  for (const value of values) {
    const normalized = value.replace(/\s+/g, ' ').trim()
...
function isOptionalBrowserEditorTargetEnvironmentUnready(result: {
  ok: boolean
  data?: unknown
  summary: string
}): boolean {
  ...
  return (
    ...
    typeof recoveryPoint === 'string' &&
    recoveryPoint.startsWith('focus:') &&
    result.summary.includes('Failed to stage browser content into') &&
    evidence.some(item => item.startsWith('stageTarget='))
  )
}
